...
function createPostgres() {
  ...
  // Idle client disconnects must not crash the process.
  pool.on('error', (err) => {
    const code = err?.code || '';
    // Transient network blips (common with Supabase pooler) — ignore quietly.
    if (
      code === 'EADDRNOTAVAIL' ||
      code === 'ECONNRESET' ||
      code === 'ECONNREFUSED' ||
    ...
    console.error('Unexpected Postgres pool error:', err?.message || err);
  });
  ...
  async function migrateFeedTables(client) {
    await client.query(`
    ...
    `);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_feed_poll_options_poll ON feed_poll_options(poll_id)`);
    await client.query(`
    ...
    `);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_feed_poll_votes_option ON feed_poll_votes(option_id)`);
    await client.query(`
      DO $$
      DECLARE
        con_name text;
      BEGIN
        ...
        LOOP
          EXECUTE format('ALTER TABLE feed_posts DROP CONSTRAINT IF EXISTS %I', con_name);
        END LOOP;
        ALTER TABLE feed_posts
          ADD CONSTRAINT feed_posts_category_check
          CHECK (category IN ('celebration', 'milestone', 'announcement', 'casual', 'poll'));
      EXCEPTION
        WHEN duplicate_object THEN NULL;
        WHEN undefined_table THEN NULL;
      END $$;
  ...
  async function migratePunchLogs(client) {
    await client.query(`
  ...
  async function migrateReimbursements(client) {
    await client.query(`
      CREATE TABLE IF NOT EXISTS reimbursement_requests (
        ...
        payment_mode TEXT NOT NULL DEFAULT 'self',
  ...
  async function migrateAttendanceRegularizations(client) {
    await client.query(`
  ...
  async function migrateLeavePolicyAcknowledgements(client) {
    await client.query(`
      CREATE TABLE IF NOT EXISTS leave_policy_acknowledgements (
        id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
        policy_version TEXT NOT NULL,
        acknowledged_at TEXT NOT NULL DEFAULT to_char((now() AT TIME ZONE 'Asia/Kolkata'), 'YYYY-MM-DD HH24:MI:SS'),
  ...
  async function initSchema() {
    ...
    try {
      ...
      await migrateLeavePolicyAcknowledgements(client);
